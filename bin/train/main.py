...
if __name__ == "__main__":
    ...
    if "--easy-colab" in sys.argv or "-c" in sys.argv:
        ...
        parser.add_argument("--arg-list", "-l", type=str, default=None, nargs='*', help="Set list of parameter values, like so: -l epochs=10 architecture=standard delay=10 lr=0.004 lr_decay=0.007 seed=0")
        args = parser.parse_args()
        if args.arg_list:
            arg_list=to_dict(args.arg_list)
            if arg_list:
                print(f"* Runing easy colab: run({arg_list})")
                run_colab(**arg_list)
        else:
